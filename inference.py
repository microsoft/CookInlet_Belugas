...
def compute_mel_spectrograms_gpu(
    ...
    for audio_file_path, items in tqdm(by_sid.items(), desc="Checking files"):
        ...
        for idx, win in items:
            resolved = resolve_spectrogram_path(
                spectrograms_path,
                sound_path=win["sound_path"],
                start=win["start"],
                end=win["end"],
    ...
    for audio_file_path, items in tqdm(files_to_process.items(), desc="Processing files"):
        ...
        for global_idx, win in tqdm(items):
            ...
            # Use standard name for new files
            npy_path = resolved if os.path.exists(resolved) else os.path.join(
                spectrograms_path, spectrogram_filename(win["sound_path"], start, end)
            )
